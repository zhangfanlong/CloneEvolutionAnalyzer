...
for classNode in class_nodes:               
	...
	#设置全部属性位置，并写入静态度量	
	Metric=[classid, classnumber, classnlines, similarity, samefile, 0, 0,0, 0, 0, 0, 0, 1]
	Matrix.append(Metric)
	
#将第一个版本的克隆组信息度量写入WEKA格式文件
mkdir(outputpath)
resultsfile = outputpath + clonefile_list[0] + '.arff'
arff.dump(resultsfile, Matrix, relation="clone_group_metrics", names=['classid', 'classnumber', 'classnlines', 'classsimilarity', 'samefile', 'pattern_static', 'pattern_same','pattern_add', 'pattern_delete', 'pattern_split', 'pattern_inconsis', 'pattern_consis', 'clonelife'])
