...
{
    public class ClusteringFCM
    {
        public int mnum = 29;//所有度量维数，对应xtemp_matrix
        public int metricno = 10; //实际选择的度量维数，对应x_matrix
        public int cnum = 20;//从价值函数最小的角度来确定，也可改进为层次聚类与k均值相结合的方式，每次计算聚类中心后计算价值函数，如果继续合并两个最近的类可以使得价值函数更小那么就合并，否则就不合并，从而确定聚类个数，暂时未更改，人工确定
        public int m = 2;
        public double objval = 0.0;
        public List<List<string>> xtemp_matrix = new List<List<string>>();//（含29维度量+*代码片段信息*的样本）
       ...
       // public List<List<XmlElement>> result = new List<List<XmlElement>>();
        ...
        public void FCM()
        {
           ...
           // StreamWriter matlab = new StreamWriter(@"E:\\2", false);
            ...
            for (int i = 0; i < cnum; i++)
            {
                ...
                //}
                //matlab.Write("];" + '\n');
                for (int j = 0; j < resulttemp[i].Count; j++)
                {  
                    ...
                    for (k = 0; k < genList.Count; k++)
                    {
                        ...
                        string sourcefile = xtemp_matrix[(resulttemp[i][j])][32];
                        string file = sourcefile.Substring(0, sourcefile.IndexOf(@"/"));
                        string classid = xtemp_matrix[(resulttemp[i][j])][31];
                    ...
                    string cgnum = xtemp_matrix[(resulttemp[i][j])][31];
                    sw.Write(cgnum);
                    sw.Write(",");
                    string versionname = xtemp_matrix[(resulttemp[i][j])][32].Substring(0, (xtemp_matrix[(resulttemp[i][j])][32].IndexOf(@"/")));
                ...
                ShowXml(i,resulttemp[i], ref treeNode);
                treeNode.Expand();
        ...
        public void CreatMatrix()
        {
            ...
            for (int i = 0; i < data.Length; i++)
            {
                ...
                string sourcefile = ((XmlElement)CFcrd.ChildNodes[sourceid - 1]).Attributes[0].Value;                
                string startline = ((XmlElement)CFcrd.ChildNodes[sourceid - 1]).Attributes[1].Value;
                string endline = ((XmlElement)CFcrd.ChildNodes[sourceid - 1]).Attributes[2].Value;
                string name = ((XmlElement)CFcrd.ChildNodes[sourceid - 1]).Name;
                newlist.Add(crdpath);
                newlist.Add(classid);
                newlist.Add(sourcefile);
