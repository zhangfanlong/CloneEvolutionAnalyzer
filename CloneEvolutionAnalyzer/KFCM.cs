...
namespace CloneEvolutionAnalyzer
{
    public class KFCM
    {
        ...
        public int metricno = 9;//实际选择的度量维数，对应x_matrix
        public int cnum = 3;//从价值函数最小的角度来确定，也可改进为层次聚类与k均值相结合的方式，每次计算聚类中心后计算价值函数，如果继续合并两个最近的类可以使得价值函数更小那么就合并，否则就不合并，从而确定聚类个数，暂时未更改，人工确定
        public int m = 2;
        ...
        public List<List<double>> kxv = new List<List<double>>();
        ...
        public void kFCM()
        {
            ...
            Init();//随机初始化隶属度矩阵
            Computekxv(); //计算聚类中心
            objval = objectfun(u_matrix, kxv, x_matrix, cnum, (x_matrix.Count - 1), metricno, m);//计算优化的目标函数
            //迭代重置隶属矩阵与聚类中心
            ...
            do
            {
                for (int i = 0; i < (x_matrix.Count - 1); i++)
                {
                    ...
                    for (int j = 0; j < cnum; j++)
                    {
                        ...
                        {
                            temp += System.Math.Pow((1 - kxv[t][i]), -1 / (m - 1));
                        }

                        u_matrix[j][i] = System.Math.Pow((1 - kxv[j][i]), -1 / (m - 1)) / temp;
                    }
                }

                temp = objectfun(u_matrix, kxv, x_matrix, cnum, (x_matrix.Count - 1), metricno, m);
                ...
                for (int i = 0; i < cnum; i++)
                {
                    for (int j = 0; j < (x_matrix.Count - 1); j++)
                    {
                        ...
                        for (int k = 0; k < (x_matrix.Count - 1); k++)
                        {
                            ...
                            {
                                v1[t] = double.Parse(x_matrix[k + 1][t + 1]);
                                v2[t] = double.Parse(x_matrix[j + 1][t + 1]);
                            }
                            temp0 += System.Math.Pow(u_matrix[i][j], m) * kxx(v1, v2, 2);
                        }
                        kxv[i][j] = temp0;
                        temp0 = 0.0;
                        for (int t = 0; t < (x_matrix.Count - 1); t++)
                            temp0 += System.Math.Pow(u_matrix[i][t], m);
                        kxv[i][j] /= temp0;
                    }
                }
                n_cycle++;
            ...
            //            v2[k] = center[j][k];
            //        }
            //        double dis = System.Math.Sqrt(1 - kxx(v1,v2,2));
            //        if (dis < min)
            //            min = dis;
            //    }
            //}
            ...
            for (int i = 0; i < cnum; i++)
            {
                for (int j = 0; j < (x_matrix.Count - 1); j++)
                {
                    ...
                    for (int k = 0; k < (x_matrix.Count - 1); k++)
                    {
                        ...
                        temp += System.Math.Pow(u_matrix[i][j], m) * kxx(v1, v2, 2);                        
                    }                                           
                    kxv[i][j] = temp;
                    temp = 0.0;
                    for (int t = 0; t < (x_matrix.Count - 1); t++)
        ...
        //计算优化的目标函数
        double objectfun(List<List<double>> u_matrix, List<List<double>> kxv, List<List<string>> x_matrix, int cum, int n, int metricno, int m)
        {            
            double value = 0.0;
            for (int i = 0; i < cnum; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    value += System.Math.Pow(u_matrix[i][j], m) * (1 - kxv[i][j]);
                }
            }
            return value;
        }
        ...
        double kxx(List<double> v1, List<double> v2, int a)
        {
            double result = 0.0;
            for (int i = 0; i < metricno; i++)
            {
